...
- Cheap-stage verdict on recurrence:
  ...
  - recurrence no longer looks "dead"; it looks neutral-but-live, with `d=640` as the current cheap anchor
  - the next cheapest discriminator is one `KV4 -> KV2` reallocation on the `d=640` line before spending an honest `8`-GPU comparison run
...
- The first working runtime surface is now lean idea-testing rather than `8`-GPU reproduction:
  ...
  - `1993` validated the warmed substrate and jobs `2000`, `2003`, and `2004` completed the width-selection sweep for the first recurrence line
...
6. Neutral-stable follow-up
   - Run `rec_u3_r3_d640_kv2_mlp2_smoke1gpu`.
   - Goal: test whether cheap KV-head reallocation can close the remaining `~0.0080` cheap-stage gap without giving back the recurrence byte win.
...
8. Promoted full-contract recurrence sweep
   ...
   - `rec_u3_r3_d640_kv2_mlp2` only if the cheap `KV4 -> KV2` test stays live
9. Post-sweep decision rule
   ...
   - Ambiguous: keep the `d=640` recurrence anchor and run exactly one cheap `KV4 -> KV2` reallocation before switching families
...
| `rec_u3_r3_d640_kv2_mlp2_smoke1gpu` | recurrence-smoke | `3` unique blocks, `3` repeats, `d=640`, `KV2`, `MLP2`, lean `1`-GPU envelope | test cheap KV-head reallocation on the promoted recurrence anchor | pending | next cheap discriminator before an honest `8`-GPU comparison |
...
| `rec_u3_r3_d640_kv2_mlp2` | recurrence | `3` unique blocks, `3` repeats, `d=640`, `KV2`, `MLP2` | promote cheap KV-head reallocation if the smoke stays live | pending | only run after the cheap `KV4 -> KV2` check |
...
1. Cheap `KV4 -> KV2` reallocation on the `d=640` recurrence anchor
